...
use jinowom\yii2wtools\tools\FileHelper;
use Yii;
...
{
    public $showName = 'jinowom wajaxcrud generator';
    public $modelClass;
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['controllerClass', 'modelClass', 'searchModelClass', 'formModelClass', 'baseControllerClass', 'viewPath', 'editableFields', 'dateRangeFields', 'rangeFields', 'thumbImageFields', 'enumFields'], 'filter', 'filter' => 'trim'],
            ...
            ['viewPath', 'safe'],
            [['editableFields', 'dateRangeFields', 'rangeFields', 'thumbImageFields', 'enumFields',], 'checkResuing'],
        ]);
    }
     ...
     * @return bool
     */
    public function checkResuing($attribute, $params)
    {
        $class = $this->modelClass;
        $fields = explode(',', $this->$attribute);
        $pk = $class::primaryKey();
        foreach ($fields as $k => $v) {
            if (!$v)continue;
            $v = trim($v);
            if (in_array($v, $pk)) {
                $this->addError($attribute, "can not be primary key(s)");
            }
            if (!in_array($v, (new $class)->attributes())) {
                $this->addError($attribute, "field '{$v}' not found!");
            }
        }
        $allFields = ArrayHelper::arr2str([
            $this->dateRangeFields,
            $this->rangeFields,
            $this->editableFields,
            $this->thumbImageFields,
            $this->enumFields,
        ]);
        $allFieldsArr = ArrayHelper::str2arr($allFields);
        $uniqueFields = array_unique($allFieldsArr);
        if (count($allFieldsArr) > count($uniqueFields)){
            $this->addError($attribute, "field may be re used");
        }
    }
    ...
    public function generateSearchRules()
    {
        ...
        foreach ($table->columns as $column) {
            if (in_array($column->name, ArrayHelper::str2arr($this->dateRangeFields))){
                // [['created_at'], 'match', 'pattern' => '/^.+\s\-\s.+$/'],
                $types['match']['columns'][] = $column->name;
                $types['match']['pattern'] = '/^.+\s\-\s.+$/';
            }elseif (in_array($column->name, ArrayHelper::str2arr($this->rangeFields))){
                $types['match']['columns'][] = $column->name;
                $types['match']['pattern'] = '/^.+\s\-\s.+$/';
            }else{
                switch ($column->type) {
    ...
    public function generateSearchConditions()
    {
        ...
        foreach ($columns as $column => $type) {
            if (in_array($column, ArrayHelper::str2arr($this->dateRangeFields))){
                $conditions[] = "\$this->rangeFilter(\$query, '{$column}', true);\n";
            }elseif (in_array($column, ArrayHelper::str2arr($this->rangeFields))){
                $conditions[] = "\$this->rangeFilter(\$query, '{$column}');\n";
            }else{
                switch ($type) {
...
//                        $hashConditions[] = "'{$column}' => \$this->{$column},";
                        $conditions[] = "\$this->fieldFilter(\$query, '{$column}');\n";
                        break;
                    default:
                        $likeConditions[] = "\$this->fieldFilter(\$query, '{$column}', 'like');\n";
                        break;
                }
            }
        }
